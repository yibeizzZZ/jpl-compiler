...
class Stack:
    ...
    def unalign(self) -> List[str]:
        ...
        if not self.padding_stack:
            instructions.append(";Nothing in Padding")
            return instructions
    ...
    def push(self, value: str, size: int, description: str = "") -> List[str]:
        ...
        instructions = []
        if padded_size == 8:
            instructions.append(f"push {value} ; {description} actually {size} bytes, new offset: {new_offset},now have {self.contents}")
        else:
    ...
    def pop(self, reg: str, size: int ) -> List[str]:
        ...
        new_offset = self.offset - padded_size
        instructions = []
        expected_desc: str = "reg"
        if not self.contents :
            print( ";pop mismatch: stack is empty")
        actual_desc, actual_size, _ = self.contents.pop()
        if (expected_desc == "" or expected_desc == actual_desc):
            print(f";pop mismatch: expected {expected_desc}, got {actual_desc}")

        if padded_size == 8:
            instructions.append(f"pop {reg} ; {actual_desc}, actually {size} bytes, new offset: {new_offset},now have {self.contents}")
        else:
    ...
    def push_reg(self, reg: str, size: int, comment: str = "") -> List[str]:
        ...
        return [f"push {reg}{comment_str}; new offset: {new_offset} ,now have {self.contents}"]

    def pop_reg(self, reg: str, size: int, comment: str = "") -> List[str]:
        new_offset = self.offset - size
        if self.contents and self.contents[-1][0] == reg:
             print(f";pop_reg mismatch: expected {reg}, at size {size}")
        self.contents.pop()
        self.offset = new_offset
        comment_str = f" ; {comment}" if comment else ""
        return [f"pop {reg}{comment_str}; new offset: {new_offset} ,now have {self.contents}"]
    ...
    def peek(self) -> Tuple[str, int, int]:
        if not self.contents:
            print(";DEBUG: Stack is empty! Current offset:", self.offset)
            # 返回一个默认值，或者你也可以返回 None
            return ("<empty>", 0, 0)
